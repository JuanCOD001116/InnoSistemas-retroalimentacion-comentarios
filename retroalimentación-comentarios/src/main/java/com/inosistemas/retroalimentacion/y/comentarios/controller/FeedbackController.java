...
public class FeedbackController {
    ...
    private boolean isProfessor(String roleHeader) {
        // Si el header dice "profesor", se usa, sino false - no hay restricciones
        // reales
        return roleHeader != null && roleHeader.equalsIgnoreCase("profesor");
    }

    // ENDPOINT COMPLETAMENTE ABIERTO - Cualquier usuario puede acceder sin
    // restricciones
    @GetMapping("/feedback")
    ...
    // ENDPOINT PARA ADMINISTRACIÓN - Listar TODOS los feedbacks sin filtros
    @GetMapping("/feedback/all")
    public List<Feedback> listAllFeedbacks() {
        // Sin restricciones - útil para administración, testing, o dashboards
        return service.listAll();
    }

    // ENDPOINT PARA ADMINISTRACIÓN - Listar TODAS las respuestas sin filtros
    @GetMapping("/feedback-responses/all")
    ...
    public ResponseEntity<Feedback> create(@RequestBody CreateFeedbackRequest req,
        ...
        Feedback created = service.createFeedback(userId(principal, userIdHeader), isProfessor(role), req.projectId(),
                req.taskId(), req.deliveryId(), req.content());
        return ResponseEntity.ok(created);
    }

    // ENDPOINT COMPLETAMENTE ABIERTO - Cualquier usuario puede actualizar cualquier
    // feedback
    ...
    // ENDPOINT COMPLETAMENTE ABIERTO - Cualquier usuario puede eliminar cualquier
    // feedback
    ...
    public ResponseEntity<FeedbackResponse> createResponse(@PathVariable long id, @RequestBody UpdateRequest req,
        ...
        FeedbackResponse created = service.createResponse(userId(principal, userIdHeader), isProfessor(role), id,
                req.content());
        return ResponseEntity.ok(created);
    }
